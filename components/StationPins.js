...
const StationPins = (props) => {
  ...
  return geojson.map((geo, i) => {
    if (geo?.geometry?.coordinates?.[0] === undefined || geo?.geometry?.coordinates?.[1] === undefined ) return ''
    ...
    const textTransform = renderedNumber < 10 ? "translate(-3px, 1px)" : "translate(-6px, 1px)"
    ...
    return (
      ...
      >
        ...
        >
          ...
          <text x="50%" y="50%" style={{ fontSize: '10px', fill: '#000', transform: textTransform }}>
            {renderedNumber}
          </text>
        </svg>
        <MdDirectionsBike
