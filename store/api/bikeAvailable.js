...
const actions = {
  fetchBikeAvailable: 'bikeStation/fetchBikeAvailable',
  fetchAdditionalBikeAvailable: 'bikeStation/fetchAdditionalBikeAvailable',
}

export const fetchBikeAvailable = createAsyncThunk(actions.fetchBikeAvailable, async () => {})
export const fetchAdditionalBikeAvailable = createAsyncThunk(actions.fetchAdditionalBikeAvailable, async () => {})

export const apiBikeAvailable = () => {
  ...
  const fetchFactory = (url, action = actions.fetchBikeAvailable, resolve = emptyFunc, reject = emptyFunc) => {
  ...
  const get = () => {
    if (_city && !_isLoading && hasMore()) {
      ...
      const action = _gotPage === 0 ? actions.fetchBikeAvailable : actions.fetchAdditionalBikeAvailable
      const url = `https://ptx.transportdata.tw/MOTC/v2/Bike/Availability/${_city}?$top=${EntityLimit}${querySkip}&$format=JSON`
