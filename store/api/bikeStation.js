...
const actions = {
  fetchBikeStation: 'bikeStation/fetchBikeStation',
  fetchAdditionalBikeStation: 'bikeStation/fetchAdditionalBikeStation',
}

export const fetchBikeStation = createAsyncThunk(actions.fetchBikeStation, async () => {})
export const fetchAdditionalBikeStation = createAsyncThunk(actions.fetchAdditionalBikeStation, async () => {})

export const apiBikeStation = () => {
  ...
  const fetchFactory = (url, action = actions.fetchBikeStation, resolve = emptyFunc, reject = emptyFunc) => {
  ...
  const get = () => {
    if (_city && !_isLoading && hasMore()) {
      ...
      const action = _gotPage === 0 ? actions.fetchBikeStation : actions.fetchAdditionalBikeStation
      const url = `https://ptx.transportdata.tw/MOTC/v2/Bike/Station/${_city}?$top=${EntityLimit}${querySkip}&$format=JSON`
